...
import type { CJMNode, CJMNodeData, SendTextNodeData } from "@/app/cjm-designer/page"
...
export function useCJMDesigner() {
  ...
  const createEdgesFromNodes = useCallback((nodes: CJMNode[]): Edge[] => {
    ...
    nodes.forEach((node) => {
      ...
      // Handle not_found_step for search_knowledgebase nodes
      if ((nodeData as any).not_found_step) {
        const targetExists = nodes.some((n) => n.id === (nodeData as any).not_found_step)
        if (targetExists) {
          allEdges.push({
            id: `${node.id}-not_found_step`,
            source: node.id,
            target: (nodeData as any).not_found_step,
  ...
  const onConnect: OnConnect = useCallback(
    (connection) => {
      ...
      const singleOutputHandles = new Set(["next_step", "else_step", "default_step", "exit_step", "timeout_step", "error_step", "not_found_step"])
      ...
      updateNodeAndConnections(source, (data) => {
        ...
        else if (sourceHandle === "not_found_step" && "not_found_step" in newData) (newData as any).not_found_step = target
  ...
  const nodeCreators = {
    ...
    addSearchKnowledgebaseNode: useCallback(() => addNodeAtCenter(nodeFactory.createSearchKnowledgebaseNode()), [addNodeAtCenter]),
  }
